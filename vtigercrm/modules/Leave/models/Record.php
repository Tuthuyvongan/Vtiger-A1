 ...
 */
class Leave_Record_Model extends Vtiger_Record_Model {
    ...
    public function getCreatorId($recordId){
        global $adb;
        $sql = "SELECT smcreatorid FROM vtiger_crmentity WHERE crmid = ? AND deleted = 0";
        $result = $adb->pquery($sql,array($recordId));
        $smCreator = $adb->query_result($result,0,'smcreatorid');
        return $smCreator;
    }

    public function getLeaveAccepted($employ_id){
        global $adb;
        $rs_leaved = $adb->pquery('SELECT
                                            SUM(hl.partial_day) as leaved
                                        FROM
                                            vtiger_hrm_leave as hl
                                        INNER JOIN vtiger_crmentity AS vc ON vc.crmid = hl.id
                                        WHERE vc.deleted = 0
                                        AND hl.hrm_leave_action = "Accept"
                                        AND hl.employ_id = ?',array($employ_id));
        $leaved = $adb->query_result($rs_leaved,0,'leaved');
        return $leaved;
    }
