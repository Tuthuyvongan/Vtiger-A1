 ...
 */
class Employee_Module_Model extends Vtiger_Module_Model {
    ...
    public function isCommentEnabled() {
        ...
        if(isset($_REQUEST['module']) && $_REQUEST['module']=='Employee'){
            $enabled = true;
        }
    ...
    public function getSettingLinks(){
        ...
        if($currentUser->isAdminUser()) {
            ...
            $settingsLinks[] = array(
                ...
                'linklabel' => 'HRM License Setting',
                'linkurl' => 'index.php?module=Employee&view=Setting&app=HRM',
    ...
    public function getRelationQuery($recordId, $functionName, $relatedModule, $relationId) {
        ...
        if ($_REQUEST['relatedModule']=='Leave'){
            $query = str_replace('LEFT JOIN ON .= vtiger_hrm_leave.id','',$query);
        }
...
//						3 => 'LBL_PREFER_VTIGER_RECORD',
    ...
    public function addLicense($version,$key,$info,$domain,$number_domain,$due_date){
        $adb = PearDatabase::getInstance();
        $sql = "INSERT INTO vtiger_hrm_license VALUES(?,?,?,?,?,?)";
        $adb->pquery($sql,array($version,$key,$info,$domain,$number_domain,$due_date));
    }

    public function getLisences(){
    ...
    public function check_active_domain($_license_key){
        $_api_url = "https://cmsmart.net/checkactive";
        $_domain_name = base64_encode('http://cmsmart.alo'); /*This domain must be obtained from the website url active license*/
        $_link_check_status = $_api_url . '/' . $_license_key . '/' . $_domain_name;
        print_r($_link_check_status);
        $return	= json_decode(file_get_contents($_link_check_status));
        echo '<pre>';
        die();
    }

    public function active_domain($_license_key){
        ...
        $data_active	= json_decode(file_get_contents($_link_check_active),true);
        return $data_active;
    }
