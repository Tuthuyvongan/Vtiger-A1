...
Class Employee_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		$license = $moduleModel->getLisences();
		$total_record = $this->getAllEmployeeRecord();
		if($license->type=='free' && $total_record > 5){
		    echo '<div align="center" id="license_notice" style="margin-top: 12px;padding-left: 77px;margin-left: 67px;"><i style="color: red;">The license you are using is free<a style="position: fixed;text-decoration: underline !important;" href="index.php?module=Employee&view=Setting&app=HRM">, click here to upgrade to Pro Version</a></i></div>';
		    die();
        }
		...
		$employ_status = $this->getStatusForEmployee($record);

		if($employ_status=='Active'){
		    $active_selected = "selected";
        }elseif ($employ_status=='Inactive'){
            $inactive_selected = "selected";
        }else{
        }

        $viewer->assign('EMPLOY_STATUS', $employ_status);
        $viewer->assign('ACTIVE_OP', $active_selected);
        $viewer->assign('INACTIVE_OP', $inactive_selected);
		$viewer->assign('CURRENT_STATUS', $employ_status);												  
	...
	public function getStatusForEmployee($recordId){
	    global $adb;
	    $sql = "SELECT
                  vu.`status`
                FROM
                ...
                INNER JOIN vtiger_users as vu ON he.user_id = vu.id
                WHERE vc.deleted = 0
                AND he.employid = ?";
    ...
    public function getAllEmployeeRecord(){
