...
class Candidates_RelatedList_View extends Vtiger_RelatedList_View {
	function process(Vtiger_Request $request) {
		...
		if($relatedModuleName == 'Candidates'){
		    $candidateHistory = $this->getCandidateHistory($parentId);
            $viewer->assign('HISTORYS', $candidateHistory);
            return $viewer->view('CandidateHistory.tpl', $moduleName, 'true');
        }else{
            return $viewer->view('RelatedList.tpl', $moduleName, 'true');
        }
	}

	function getCandidateHistory($recordId){
	    global $adb;
	    $sql = "SELECT * FROM hrm_employee_history WHERE employ_id = ?";
	    $result = $adb->pquery($sql,array($recordId));
	    $data = array();
	    while ($row = $adb->fetchByAssoc($result)){
	        if($row['date']){
                $row['date'] = Vtiger_Datetime_UIType::getDisplayDateTimeValue($row['date']);
            }
	        $data[] = $row;
        }
        return $data;
