...
{strip}
	...
	<sp class="relatedContainer">
								...
								<td class="relatedListEntryValues" title="{strip_tags($RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME))}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>
									<span class="value textOverflowEllipsis">
										{if $RELATED_MODULE_NAME eq 'Documents' && $RELATED_HEADERNAME eq 'document_source'}
											...
											{else}
											{if in_array($HEADER_FIELD->getName(),array('current_salary','salary_after'))}
												{number_format($RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME),0,'.',',')}
												{else}
		...
		{if $RELATED_MODULE_NAME eq 'Salary' && $REVIEW_DATE neq ''}
		<div style="margin-top: 115px;" align="center">
			<span style="">
				<strong style="color: red;">Next date to review salary: {$REVIEW_DATE}</strong>
			</span>
		</div>
		{/if}
