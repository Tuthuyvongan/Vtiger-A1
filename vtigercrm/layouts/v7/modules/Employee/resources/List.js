...
Vtiger_List_Js("Employee_List_Js",{},{

	registerEmployeeStatusToggle: function () {
        var thisInstance = this;
        jQuery('#activeEmployee, #inactiveEmployee').on('click', function(e) {
            ...
            if(currentEle.attr('id') === 'activeEmployee') {
                jQuery('#inactiveEmployee').removeClass('btn-primary');
            }else {
                jQuery('#activeEmployee').removeClass('btn-primary');
            }
        ...
        jQuery('#syn_users').on('click',function (e) {

            var total_record = jQuery("#totalCount").val();
            var hrm_version = jQuery("#license_type").val();
            if(hrm_version=='free' && total_record >= 5){
                var notice = '<i style="color: red;margin-left: -110px">The license you are using is free<a style="position: fixed;text-decoration: underline !important;" href="index.php?module=Employee&view=Setting&app=HRM">, click here to upgrade to Pro Version</a></i>';
                jQuery("#license_notice").html(notice);
                return false;
            }else {
                var validate_msg = jQuery("#license_noti").val();
                if(validate_msg != ''){
                    var notice = '<i style="color: red;margin-left: -110px">'+validate_msg+'<a style="position: fixed;text-decoration: underline !important;" href="index.php?module=Employee&view=Setting&app=HRM">, click here to update license</a></i>';
                    if(hrm_version=='free' && total_record >= 5){
                        jQuery("#license_notice").html(notice);
                        return false;
                    }
    ...
    getListViewParams : function() {
        ...
        var listParams = {
            'module' : 'Employee',
            'view' : 'List',
            'status' : listViewContainer.find('.employFilter').find('button.btn-primary').data('searchvalue')
        };
        return listParams;
    },
...
//			http://stackoverflow.com/questions/26482645/number-isintegerx-which-is-created-can-not-work-in-ie
    ...
    getRecordsCount: function () {
        ...
        var status = listViewContainer.find('.employFilter').find('button.btn-primary').data('searchvalue');
    ...
    checkLicense : function () {
	    jQuery("#Employee_listView_basicAction_LBL_ADD_RECORD, #Employee_basicAction_LBL_IMPORT").on('click',function (e) {
	        var total_record = jQuery("#totalCount").val();
	        var hrm_version = jQuery("#license_type").val();
            if(hrm_version=='free' && total_record >= 5){
                var notice = '<i style="color: red;margin-left: -110px">The license you are using is free<a style="position: fixed;text-decoration: underline !important;" href="index.php?module=Employee&view=Setting&app=HRM">, click here to upgrade to Pro Version</a></i>';
    ...
    registerEvents: function () {
        ...
        this.registerEmployeeStatusToggle();
        this.checkLicense();
        this.registerListViewMainCheckBoxClickEvent(); // Add code to fix Check Count Record
        jQuery(document).ready(function () {
            var total_record = jQuery("#totalCount").val();
            var hrm_version = jQuery("#license_type").val();
            if(hrm_version=='free' || hrm_version=='' && total_record >= 5){
                jQuery("#Employee_listView_basicAction_LBL_ADD_RECORD").attr('onclick','');
                jQuery("#Employee_basicAction_LBL_IMPORT").attr('onclick','');
            }
        });
    }
});