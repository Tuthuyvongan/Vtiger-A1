...
{strip}
	...
	<input type="hidden" name="currentStatus" value="{$CURRENT_STATUS}">
				...
				<div class='fieldBlockContainer' data-block="{$BLOCK_LABEL}">
					...
					<table class="table table-borderless">
						<tr>
                            ...
                            {if $RECORD_ID && $BLOCK_LABEL eq 'LBL_PERSONAL_DETAIL'}
								<tr>
									<td class="fieldLabel alignMiddle">Status</td>
									<td class="fieldValue">
										<select class="inputElement select2 select2-offscreen" name="employ_status" id="employStatus">
											<option value="">Select an option</option>
											<option value="Active" {$ACTIVE_OP}>Active</option>
											<option value="Inactive" {$INACTIVE_OP}>Inactive</option>
										</select>
									</td>
								</tr>
							{/if}
