...
{strip}
	...
	<input type="hidden" name="leaveBalance" value="{$LEAVE_BALANCE}">
	<input type="hidden" name="currentAction" value="{$CURRENT_ACTION}">
	<input type="hidden" name="showAction" value="{$SHOW_ACTION}">
						...
						<tr>
							...
							{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
								...
								{if $FIELD_MODEL->isEditable() eq true}
									...
									{/if}
									<td class="fieldLabel alignMiddle" {if $FIELD_MODEL->get('label') =='Action'}id="leave_action_lbl" {/if}{if $FIELD_MODEL->get('label') =='HR'}id="leave_hr_lbl" {/if}>
									...
									{if $FIELD_MODEL->get('uitype') neq '83'}
										<td class="fieldValue" {if $FIELD_NAME =='hrm_leave_action'}id="leave_action"{/if}{if $FIELD_NAME =='hr'}id="leave_hr"{/if} {if $FIELD_MODEL->getFieldDataType() eq 'boolean'} style="width:25%" {/if} {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
											{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE)}
											{if $FIELD_NAME =='partial_day'}
												<p style="margin-top: 10px;color: red;" id="leave_text">Leave balance: {$LEAVE_BALANCE}</p>
											{/if}
										</td>
									{/if}
								{/if}
