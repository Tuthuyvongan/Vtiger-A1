	...
	<div name='editContent'>
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name=blockIterator}
			{if $BLOCK_FIELDS|@count gt 0}
				{if $BLOCK_LABEL eq 'LBL_RANKING_INFORMATION'}
					<div class='fieldBlockContainer' data-block="{$BLOCK_LABEL}">
						...
						<table class="table table-borderless">
							...
							<tr>
								<td class="fieldLabel alignMiddle" style="width:1%;">
                                    {vtranslate($FIELD_MODEL->get('label'), $MODULE)}
								</td>
								<td class="fieldValue" {if $FIELD_MODEL->getFieldDataType() eq 'boolean'} style="width:25%" {/if} {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {/if}>
					...
					<div class='fieldBlockContainer' data-block="{$BLOCK_LABEL}">
						...
						<table class="table table-borderless">
							<tr>
                                ...
                                {/if}
								<td class="fieldLabel alignMiddle" {if $FIELD_MODEL->get('label') eq 'Bonus' or $FIELD_MODEL->get('label') eq 'Fined'}style="width: 1% !important;" {/if}> {*Custom for Bonus Fined Field*}
