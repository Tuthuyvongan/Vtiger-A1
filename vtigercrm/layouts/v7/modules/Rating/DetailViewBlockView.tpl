...
{strip}
	...
	{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE}
		...
		{if $BLOCK_LABEL_KEY eq 'LBL_RANKING_INFORMATION'}
			...
			<input type=hidden name="timeFormatOptions" data-value='{$DAY_STARTS}' />
				...
				<div class="blockData">
					<table class="table detailview-table no-border">
						...
						<tr>
							<td style="width:1%;" class="fieldLabel {$WIDTHTYPE}">{vtranslate({$FIELD_MODEL->get('label')},{$MODULE_NAME})}</td>
							<td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}">{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getDetailViewTemplateName(),$MODULE_NAME) FIELD_MODEL=$FIELD_MODEL USER_MODEL=$USER_MODEL MODULE=$MODULE_NAME RECORD=$RECORD}</td>
						</tr>
                        {/foreach}
					</table>
				</div>
					...
					<table class="table detailview-table no-border">
						...
						<tr>
							...
							<td class="fieldLabel textOverflowEllipsis {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldLabel_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->getName() eq 'description' or $FIELD_MODEL->get('uitype') eq '69'} style='width:8%'{/if}
									{if $FIELD_MODEL->get('label') eq 'Bonus' or $FIELD_MODEL->get('label') eq 'Fined'}style="width: 1% !important;"{/if}> {*Custom for Bonus Fined field*}
                            ...
                            {/if}
							{*Custom for Bonus Fined Field*}
                            {if $MODULE eq 'Rating' and $FIELD_MODEL->get('name') eq 'bonus'}
								</tr><tr>
                            {/if}
                            {*Custom for Bonus Fined Field*}
                            {/foreach}
