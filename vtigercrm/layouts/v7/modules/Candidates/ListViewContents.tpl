					...
					{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
						...
						<td class="listViewEntryValue" data-name="{$LISTVIEW_HEADER->get('name')}" title="{$LISTVIEW_ENTRY->getTitle($LISTVIEW_HEADER)}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}">
									...
									{else}
										...
										{else if $LISTVIEW_HEADER->getFieldDataType() eq 'picklist'}
											...
											{else}
												...
												{/if}
											{/if}
											<span {if !empty($LISTVIEW_ENTRY_VALUE)} class="picklist-color picklist-{$PICKLIST_FIELD_ID}-{Vtiger_Util_Helper::convertSpaceToHyphen($LISTVIEW_ENTRY_RAWVALUE)}" {/if}>
												{if $LISTVIEW_HEADER->get('name') eq 'candidate_action' and in_array($LISTVIEW_ENTRY->get('candidate_action'),array('Reject after shortlist','Reject after SI','Reject after MIP','Reject after OJ')) }Reject{else}{$LISTVIEW_ENTRY_VALUE}{/if}
										...
										{else if $LISTVIEW_HEADER->getFieldDataType() eq 'multipicklist'}
											...
											{/foreach}
										{else}
										{if $MODULE eq 'KPI' && $LISTVIEW_HEADERNAME eq 'cf_07'}
                                            {number_format(round($LISTVIEW_ENTRY_VALUE,-3))}
											{else}
                                            {$LISTVIEW_ENTRY_VALUE}
										{/if}
										{/if}
							...
							{if $LISTVIEW_HEADER->isEditable() eq 'true' && $LISTVIEW_HEADER->isAjaxEditable() eq 'true'}
								{if !in_array($LISTVIEW_HEADER->get('name'),array('status','candidate_action'))}
