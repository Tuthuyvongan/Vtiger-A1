...
{strip}
	...
	<input type="hidden" name="current_status" value="{$CURRENT_STT}">
						...
						<tr>
							...
							{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
								...
								{if $FIELD_MODEL->isEditable() eq true}
									...
									{/if}
									<td class="fieldLabel alignMiddle"{if $FIELD_MODEL->get('label') =='Status'}id="can_stt_lbl"{/if}{if $FIELD_MODEL->get('label') =='Action'}id="can_action_lbl"{/if}>
									...
									{if $FIELD_MODEL->get('uitype') neq '83'}
										<td class="fieldValue" {if $FIELD_NAME =='candidate_status'}id="candidate_status"{/if}{if $FIELD_NAME =='candidate_action'}id="candidate_action"{/if}{if $FIELD_MODEL->getFieldDataType() eq 'boolean'} style="width:25%" {/if} {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                            ...
                            {if $RECORD_ID and $BLOCK_LABEL eq 'LBL_CANDIDATE_INFORMATION'}
								{if in_array($CURRENT_STT,array('Inactive','Failed'))}
									<tr>
										<input type="hidden" name="currentStt" value="{$CURRENT_STT}">
										<td class="fieldLabel alignMiddle">Back to Active Candidates</td>
										<td class="fieldValue">
											<input type="checkbox" name="active_candidate" id="active_candidate" value="0">
										</td>
									</tr>
								{/if}
                        	{/if}
