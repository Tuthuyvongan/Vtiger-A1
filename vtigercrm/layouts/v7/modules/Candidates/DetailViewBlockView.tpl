...
{strip}
	...
	{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE}
		...
		{if in_array($BLOCK_LABEL_KEY,$HIDE_BLOCKS)}{*Hide block and fields *}
			{continue}
		{/if}{*Hide block and fields *}
							...
							{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
								...
								{else}
									...
									<td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $fieldDataType eq 'reminder' or $fieldDataType eq 'recurrence'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
										...
										<span class="value" {if $FIELD_NAME eq 'candidate_action'} style="background: orange;" {/if} data-field-type="{$FIELD_MODEL->getFieldDataType()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '21'} style="white-space:normal;"{/if}>
											{if $FIELD_MODEL->getName()=='candidate_action' && in_array($FIELD_MODEL->get('fieldvalue'),array('Reject after shortlist','Reject after SI','Reject after MIP','Reject after OJ'))}
												Reject
											{else}
										...
										{if $IS_AJAX_ENABLED && $FIELD_MODEL->isEditable() eq 'true' && $FIELD_MODEL->isAjaxEditable() eq 'true'}
											...
											{if $FIELD_NAME neq 'candidate_action'}
